...
CustData <- read.csv("FSOP_Customer_Data_v2.0.csv")
...
``` {r data split - cust, warning = FALSE,message = FALSE}
...
Cust_Model_Set <- New_Cust2[,!names(New_Cust2) %in% c("CUSTOMER_NUMBER_BLINDED","ON_BOARDING_DATE","New_Onboard", "cust_end_date", "Cust_Age_Yrs")]
...
set.seed(123)
LETSTrain <- createDataPartition(Cust_Model_Set$Age_Group, p=0.5, list=FALSE)
...
``` {r initial NB model, warning = FALSE,message = FALSE}
...
df <- Cust_Model_Set
target <- 15 #index of Age_Group variable
nFolds <- 3
seedVal <- 123
...
##  hyperparameter combinations to 10
...
```
I specified very low-compute intensive hyperparameters because I'm running into compute issues. Although, this may be a useful model.
